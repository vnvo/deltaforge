...
- **Elasticsearch sink** (`type: elasticsearch`) — streams CDC into Elasticsearch over the HTTP `_bulk` API as a **current-state mirror**. Upsert by primary key (`_id` from the source PK, or configurable `id_fields`) with `version_type=external` keyed on the source LSN/binlog position, so retries and out-of-order batches converge to the source's current state; a `409 version_conflict` is treated as already-superseded (success), not a failure. Deletes remove the document. **Per-table index templating** (`index: "cdc-{db}.{table}"`) — one sink spans many indices. **Explicit index mapping generated from the source schema** (`auto_create_index: true` default): decimals → `scaled_float`, dates → `date`, json → `flattened`, strings → `text`+`keyword` — avoiding dynamic-mapping mistypes. Basic and API-key auth + TLS. Delivery is at-least-once and idempotent; per-row DLQ isolation; reuses the existing sink scaffolding, batching, and metrics. 27 unit + 3 live testcontainers integration tests. Docs at `docs/src/sinks/elasticsearch.md`.
- **ClickHouse sink** (`type: clickhouse`) — streams CDC into ClickHouse over HTTP + RowBinary (exact int/decimal/datetime fidelity). One uniform write path (`source columns + _op/_version/_deleted/_source_ts`); the target table engine selects behaviour — **change-log** (`MergeTree`) or **current-state upsert** (`ReplacingMergeTree(_version, _deleted)`), where ClickHouse's merge gives mirror-the-source semantics with no app-side merge. **Auto table creation** from the source DDL (engine + `ORDER BY` from mode + PK; `auto_create: true` default). `_version` derives from the source LSN/binlog position (monotonic). Delivery is at-least-once; upsert mode is idempotent/self-correcting; change-log dedups replayed batches via `insert_deduplication_token`. Per-row DLQ isolation; reuses the existing sink scaffolding, batching, and metrics. Docs at `docs/src/sinks/clickhouse.md`.
- **Filter processor now configurable** — the predicate-based filter processor (drop events by op type, table pattern, or field predicate: `eq`/`ne`/`gt`/`gte`/`lt`/`lte`/`in`/`not_in`/`contains`/`regex`/`changed`/`exists`) is now wired into the pipeline processor chain as `type: filter`. The implementation and tests already existed but had no config path. `ops`/`tables`/`fields` are all optional — omit any to skip that check. Distinct from sink-level `filter:` (`SinkFilter`), which routes by synthetic/producer only.
- **Pipeline-level `sink_batch_deadline_secs`** — coordinator-level safety net that bounds how long the coordinator awaits any single sink's `send_batch`. Defense in depth in addition to each sink's own internal timeout: catches the sink itself misbehaving (deadlock, infinite retry loop, slow Rust code) rather than just its underlying library. Timeouts become `SinkError::Backpressure` and are routed per `required`. Metric: `deltaforge_coordinator_sink_timeout_total`. `None` (disabled) by default; recommended for production. See [Guarantees & Correctness — Coordinator-level deadline](docs/src/guarantees.md#coordinator-level-deadline-defense-in-depth).
- **S3 / Parquet / JSON Lines sink** — direct lakehouse path out of DeltaForge. Writes CDC events to S3-compatible object storage (AWS S3, MinIO, GCS, Azure Blob, R2, Wasabi, local FS) as Parquet (default) or JSON Lines. JSONL supports gzip and zstd compression via a streaming `async-compression` encoder so memory stays bounded (~8 MiB BufWriter chunk) instead of holding the full file in RAM. PG arrays land as native Arrow `List<T>` columns (scalar element types) and hstore as `Map<Utf8, Utf8>` — both queryable as nested columns in Athena/Trino/DuckDB. DDL-derived Arrow schemas with native `Decimal128(p, s)` support — fixes TD-003 on the Parquet path. Hive-style partitioning: `{prefix}/table={t}/year={y}/month={mm}/day={dd}/<ulid>.parquet`. File rolling on size / event count / wall-clock age / idle window (defaults 256 MiB / 1M / 5 min / 10 min). Atomic multipart commit — readers never see partial files. Per-partition writer pool with concurrent close. Per-batch `send_timeout_secs` bounds worst-case wait. **Per-row DLQ via slow-path retry** — a bad row is isolated into `BatchResult.dlq_failures` and routed to the DLQ; good rows in the same batch are written normally. Production prerequisite: configure `AbortIncompleteMultipartUpload: 1 day` lifecycle policy on the target bucket. 50 sink unit tests, 4 runner E2E tests against MinIO, 1 MiniStack canary verifying AWS-shape parity, manual chaos procedures via Toxiproxy. Sample configs in `chaos/config/{mysql-to-s3,pg-to-s3-jsonl}.yaml`. Full sink doc at `docs/src/sinks/s3.md`. See [docs/specs/s3-parquet-sink.md](docs/specs/s3-parquet-sink.md) for the RFC.
...
- **S3 sink chaos scenarios** — `s3-soak` (sustained Parquet write load; a stability test, not a throughput benchmark), `s3-outage` (cuts the MinIO proxy mid-stream, verifies backpressure and recovery), and `s3-backlog-drain` — a **sink-agnostic** backlog-drain benchmark that measures catch-up throughput via the coordinator's `deltaforge_sink_events_total` metric rather than a Kafka offset, so it works for any sink. The backlog-drain benchmark was generalized to support both measurement modes (Kafka offset and sink metric).
...
- **Dependency updates** — `pgwire-replication` 0.3.1 → 0.4.0, `mysql-binlog-connector-rust` 0.3.2 → 0.3.3, and ~250 transitive crates updated to latest semver-compatible versions (tokio, axum, clap, serde_json, tracing, reqwest, etc.). `v8` pinned to 140.0.0 to keep the deno_core / serde_v8 / temporal_rs chain on a known-good set. Removed unused local `pgwire-replication/` development copy. The 0.4.0 bump adds standby-status/keepalive feedback while the event channel is full (prevents connection resets under backpressure), pull-based replication metrics, and multi-publication slots; `ReplicationConfig` is now `#[non_exhaustive]`, so construction migrated to the `new()` + `with_*()` builder API. The Postgres source stays on text wire format (0.4.0's opt-in binary output is not enabled).

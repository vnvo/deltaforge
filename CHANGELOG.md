...
- **Dependency updates** — `pgwire-replication` 0.3.1 → 0.3.2, `mysql-binlog-connector-rust` 0.3.2 → 0.3.3, and ~250 transitive crates updated to latest semver-compatible versions (tokio, axum, clap, serde_json, tracing, reqwest, etc.). `v8` pinned to 140.0.0 to keep the deno_core / serde_v8 / temporal_rs chain on a known-good set. Removed unused local `pgwire-replication/` development copy.

...
- **S3 / Parquet / JSON Lines sink** — direct lakehouse path out of DeltaForge. Writes CDC events to S3-compatible object storage (AWS S3, MinIO, GCS, Azure Blob, R2, Wasabi, local FS) as Parquet (default) or JSON Lines (gzip optional). DDL-derived Arrow schemas with native `Decimal128(p, s)` support — fixes TD-003 on the Parquet path. Hive-style partitioning: `{prefix}/table={t}/year={y}/month={mm}/day={dd}/<ulid>.parquet`. File rolling on size / event count / wall-clock age / idle window (defaults 256 MiB / 1M / 5 min / 10 min). Atomic multipart commit — readers never see partial files. Per-partition writer pool with concurrent close. Production prerequisite: configure `AbortIncompleteMultipartUpload: 1 day` lifecycle policy on the target bucket. 47 sink unit tests, 4 runner E2E tests against MinIO, 1 MiniStack canary verifying AWS-shape parity, manual chaos procedures via Toxiproxy. Sample configs in `chaos/config/{mysql-to-s3,pg-to-s3-jsonl}.yaml`. Full sink doc at `docs/src/sinks/s3.md`. See [docs/specs/s3-parquet-sink.md](docs/specs/s3-parquet-sink.md) for the RFC.

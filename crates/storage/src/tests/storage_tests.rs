...
mod tests {
    ...
    use deltaforge_storage::{
        ArcStorageBackend, MemoryStorageBackend, StorageBackend,
        adapters::{BackendCheckpointStore, DurableSchemaRegistry},
    };

    #[cfg(feature = "sqlite")]
    use deltaforge_storage::SqliteStorageBackend;
    ...
    async fn queue_ack_removes_up_to_id_inclusive() {
        for_each_backend!(|b: ArcStorageBackend| async move {
            ...
            // Peek first 3
            let peeked =
                b.queue_peek("quarantine", "pipe/tbl", 3).await.unwrap();
    ...
    async fn schema_registry_cold_start_replay() {
        for_each_backend!(|b: ArcStorageBackend| async move {
            ...
            assert_eq!(
                versions[0].checkpoint.as_deref(),
                Some(b"cp1".as_slice())
            );
    ...
    async fn schema_registry_get_at_sequence() {
        for_each_backend!(|b: ArcStorageBackend| async move {
            ...
            let sv =
                reg.get_at_sequence("t", "db", "tbl", seq_after_v1).unwrap();

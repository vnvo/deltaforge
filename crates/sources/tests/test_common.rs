...
pub static PG_CONTAINER: OnceCell<(ContainerAsync<GenericImage>, u16)> = OnceCell::const_new();
...
async fn pg_setup_cdc_user(port: u16) -> Result<()> {
    let (client, conn) =
        tokio_postgres::connect(&pg_admin_dsn_on("postgres", port), NoTls).await?;
...
fn pg_admin_dsn_on(dbname: &str, port: u16) -> String {
    format!("host=127.0.0.1 port={port} user={PG_USER} password={PG_PASS} dbname={dbname}")
}

fn pg_cdc_dsn_on(dbname: &str, port: u16) -> String {
    format!("host=127.0.0.1 port={port} user={PG_CDC_USER} password={PG_CDC_PASS} dbname={dbname}")
}
...
pub async fn pg_create_db(prefix: &str) -> Result<(String, tokio_postgres::Client)> {
...
pub async fn pg_setup(suffix: &str) -> Result<(String, tokio_postgres::Client)> {
    init_test_tracing();
...
pub async fn pg_make_registry() -> Arc<DurableSchemaRegistry> {
...
pub async fn pg_make_schema_loader(dsn: &str) -> Result<PostgresSchemaLoader> {
    Ok(PostgresSchemaLoader::new(dsn, pg_make_registry().await, "test"))
}

/// Convenience: build a schema loader using the admin DSN for a given db.
pub async fn pg_make_schema_loader_for(dbname: &str) -> Result<PostgresSchemaLoader> {
